...
class ConsistentVersionsPluginIntegrationTest {
    ...
    private void makePlatformPom(MavenRepo repo, String group, String name, String version) {
        File repoDir = repo.path().toFile();
        File dir = new File(repoDir, group + "/" + name + "/" + version);
        dir.mkdirs();
        File pomFile = new File(dir, name + "-" + version + ".pom");
        try {
            ...
            throw new java.io.UncheckedIOException("Failed to write platform POM", e);
        }
    }
    ...
    void virtual_platform_is_respected_across_projects(
        ...
        rootProject.file("versions.lock").assertThat().hasContent(expectedLock);
    }
    ...
    void write_locks_and_verify_locks_work_in_the_presence_of_versions_props_constraints(
        ...
        rootProject
                ...
                .appendProperty("org1:platform", "1.0")
                .appendProperty("org2:*", "1.0")
                .appendProperty("org.slf4j:slf4j-api", "1.7.25");
    ...
    void versions_props_contents_do_not_get_published_as_constraints(
        ...
        rootProject
                .propertiesFile("versions.props")
                .appendProperty("org.slf4j:*", "1.7.25")
                .appendProperty("ch.qos.logback:*", "1.1.11")
                .appendProperty("should:not-publish", "1.0");
        ...
        File fooMetadataFilename =
                foo.buildDir().file("publications/maven/module.json").path().toFile();
        MetadataFile fooMetadata;
        try {
            fooMetadata = new ObjectMapper().readValue(fooMetadataFilename, MetadataFile.class);
        } catch (IOException e) {
            throw new java.io.UncheckedIOException("Failed to read metadata file", e);
        }

        Set<MetadataFile.Variant> expectedVariants = Set.of(
        ...
        org.assertj.core.api.Assertions.assertThat(fooMetadata.variants()).isEqualTo(expectedVariants);
    }
    ...
    void works_with_included_builds(GradleInvoker gradle, RootProject rootProject, MavenRepo repo) {
        // add included build
        Directory includedBuild = rootProject.directory("included-build");
        rootProject.settingsGradle().append("""
            includeBuild 'included-build'
            """);

        // configure the included build
        includedBuild.file("settings.gradle").overwrite("""
        ...
        includedBuild.gradleFile("build.gradle").plugins().add(PLUGIN_NAME);
        includedBuild.file("build.gradle").append("""
            allprojects {
            ...
            """, repo.path().toUri(), repo.path());

        Directory innerA = includedBuild.directory("innerA");
        innerA.gradleFile("build.gradle").plugins().add("java").add("maven-publish");
        innerA.file("build.gradle").append("""
            dependencies {
                implementation 'org.slf4j:slf4j-api'
                runtimeOnly 'ch.qos.logback:logback-classic:1.1.11' // brings in slf4j-api 1.7.22
            }
        ...
        Directory innerB = includedBuild.directory("innerB");
        innerB.gradleFile("build.gradle").plugins().add("java").add("maven-publish");
        innerB.file("build.gradle").append("""
            dependencies {
                implementation 'test-alignment:module-with-higher-version'
            }
            """);

        includedBuild.file("versions.props").overwrite("""
            org.slf4j:slf4j-api = 1.7.25
            test-alignment:* = 1.1
            """);

        // configure main build
        rootProject.buildGradle().plugins().add("java").add("maven-publish");
        rootProject.buildGradle().append("""
        ...
        rootProject.propertiesFile("versions.props").appendProperty("test-alignment:*", "1.0");
        ...
        includedBuild.file("versions.lock").assertThat().hasContent(expectedInnerLock);
        ...
        rootProject.file("versions.lock").assertThat().hasContent(expectedRootLock);
        ...
        org.assertj.core.api.Assertions.assertThat(rootVersionsLock)
